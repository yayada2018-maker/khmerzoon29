...
const VideoPlayer = ({ 
  ...
  useEffect(() => {
    ...
    if (displayedSources && displayedSources.length > 0) {
      ...
      if (shouldUpdate) {
        ...
        // IMPORTANT: Don't show loading for embed/iframe sources - they handle their own loading
        const serverType = normalizeType(defaultServer.source_type, defaultServer.url);
        const isEmbedSource = serverType === 'embed' || serverType === 'iframe';
        setIsLoading(!isEmbedSource);
        setCurrentTime(0);
        setDuration(0);
        setBuffered(0);
        setIsPlaying(false);
  ...
  return (
    ...
    >
      {/* Loading Overlay - Only show for direct playback (mp4/hls/dash), not for embed/iframe sources */}
      {isLoading && !isLocked && sourceType !== "embed" && sourceType !== "iframe" && (
