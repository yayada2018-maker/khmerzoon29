...
const NativeVideoPlayer = ({ 
  ...
  useEffect(() => {
    // For iframe sources, just ensure loading is false - they handle their own loading
  ...
  // CRITICAL: Only show when video is ACTIVELY PLAYING (not paused at start)
  // This ensures the button appears AFTER user presses play, not before
  const { activeSegment, handleSkip: handleSkipButtonClick, shouldShowButton: shouldShowSkipButton } = useSkipSegments({
    ...
    // Only show when video is actively playing - button appears after playback starts
    isVisible: isPlaying,
  });
  ...
  const handleServerChange = useCallback(async (source: VideoSource) => {
    ...
    const newSourceType = normalizeType(source.source_type, source.url);
    const wasIframe = sourceType === 'iframe';
    const isNewIframe = newSourceType === 'iframe';
    
    logNativeDebug('handleServerChange', 'Switching server', {
      from: currentServer?.server_name,
      to: source.server_name,
      fromType: sourceType,
      toType: newSourceType,
    });
    
    // Set loading state before changing source - but not for iframe sources (they handle their own loading)
    setIsLoading(!isNewIframe);
    setIsPlaying(false);
    
    // CRITICAL: Clear the last loaded source ref when switching servers
    // This ensures the video will reload properly when switching from iframe back to mp4/hls
    ...
    // If we have a video element, reset it (especially important when switching from iframe)
    if (videoRef.current) {
      videoRef.current.pause();
      videoRef.current.removeAttribute('src');
      videoRef.current.load();
  ...
  const handleEpisodeSelect = useCallback(async (episodeId: string) => {
    ...
    // CRITICAL: Clear last loaded source ref to allow fresh loading
    lastLoadedSourceRef.current = null;
  ...
  return (
    <>
      ...
      >
        ...
        {/* Loading Overlay - Only show for direct playback (mp4/hls), not for embed/iframe sources */}
        {isLoading && !isLocked && sourceType !== 'iframe' && (
