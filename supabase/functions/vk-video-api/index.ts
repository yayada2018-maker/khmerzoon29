...
serve(async (req) => {
  ...
  try {
    ...
    // Check for video_ext.php format first (including vkvideo.ru domain)
    const extMatch = videoUrl.match(/video_ext\.php\?.*oid=(-?\d+).*id=(\d+)/i);
    if (extMatch) {
      ownerId = extMatch[1];
      videoId = extMatch[2];
      // Try to get hash from URL parameters
    ...
    // If URL already contains video_ext.php format with hash, return it directly
    if (videoUrl.includes('video_ext.php') && existingAccessKey) {
      let embedUrl = videoUrl.replace(/^http:/, 'https:');
      ...
      console.log('Using direct embed URL with hash:', embedUrl);
      return new Response(
        JSON.stringify({
          success: true,
          embedUrl,
    ...
    const fallbackEmbedUrl = `https://vk.com/video_ext.php?${params.toString()}`;
